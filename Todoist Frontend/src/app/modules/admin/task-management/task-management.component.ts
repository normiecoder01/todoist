...
export class TaskManagementComponent implements OnInit {
  ...
  completedTasks: Task[] = [];  // Declare the completedTasks array
  today: Date = new Date();
  datetime24h: Date | undefined;
  tasks: Task[] = []; // Main tasks array
      ...
      const matchesSearch =
        this.searchTerm && this.searchTerm.trim() !== ''
          ? task.taskTitle.toLowerCase().includes(this.searchTerm.toLowerCase()) ||
            task.taskDescription?.toLowerCase().includes(this.searchTerm.toLowerCase())
          : true;
  
      const matchesStatus =
        this.selectedStatus !== ''
          ? task.percentageComplete === 100 === (this.selectedStatus === 'true')
          : true;
...
showCompletedTasks() {
  ...
  saveTask(): void {
    if (this.taskForm.valid) {
      ...
      const newTask = {
        ...this.taskForm.value, // This will include bucketId, taskTitle, etc.
        createdBy: loggedUser.userId,
        assignedTo: loggedUser.userId,
      };
      console.log(newTask);
  ...
  //   this.taskService.getTaskDetails(taskId).subscribe((task: Task) => {
  //     this.taskForm.patchValue(task); // Patch the form with the task data
...
deleteTask(taskId: number): void {
      ...
      this.taskService.deleteTask(taskId).subscribe(
        () => {
          ...
          this.completedTasks = this.completedTasks.filter(task => task.todoTaskId !== taskId);
