...
import { QuoteService } from '../../../core/services/quote.service'; // Adjust path as needed
...
export class DashboardComponent implements OnInit {
  ...
  fetchQuote() {
    this.quoteService.getQuoteOfTheDay().subscribe(
      ...
      (error) => {
        this.quote = "Unable to fetch the quote of the day. Please try again later.";
        this.author = '';
      }
    );
  }
  ...
  // Method to check if the current user is the task creator
  isTaskCreator(task: getIncompleteTasks): boolean {
    const loggedUser = JSON.parse(localStorage.getItem('loggedUser') || '{}');
    return task.createdByUserName === loggedUser.userName;
  }
  ...
  filterTasks(): void {
    ...
    this.filteredTasks = this.tasks.filter((task) => {
      ...
      const matchesBucket = this.selectedBucket
        ? task.bucketName === this.selectedBucket.bucketName // Use bucketName for comparison
        : true;

      return matchesSearch && matchesPriority && matchesBucket;
    });
  }
  ...
  saveTask(): void {
    if (this.taskForm.valid) {
      ...
      const taskData = {
        ...
        assignedTo:loggedUser.userId // Ensure the createdBy field is preserved
      };
      ...
      if (this.editMode) {
        ...
        this.taskService
          ...
          .subscribe(() => {
            ...
            this.displayModal = false; // Close modal
            Swal.fire({
              title: 'Updated!',
        ...
        this.taskService.createTask(taskData).subscribe(() => {
          this.loadTasks(); // Refresh tasks after creating
          this.displayModal = false; // Close modal
          Swal.fire({
            title: 'Added!',
  ...
  editTask(task: Task): void {
    ...
    // Patch the task form with the selected task's details
  ...
  // // Change the task status directly from the table with confirmation
  // changeTaskStatus(taskId: number, newStatus: boolean): void {
  //   const taskToEdit = this.tasks.find((task) => task.userTaskId === taskId);
  //   if (taskToEdit) {
  //     Swal.fire({
  //       title: 'Are you sure?',
  //       text: `Do you want to change the status of the task "${taskToEdit.taskTitle}" to "${newStatus}"?`,
  //       icon: 'warning',
  ...
  //         this.taskService.editTask(taskToEdit).subscribe(() => {
  //           Swal.fire({
  //             title: 'Changed!',
  //             text: 'Task status has been changed successfully.',
  //             icon: 'success',
  //             timer: 1500,
  //             showConfirmButton: false,
  //             position: 'top-right',
  //             toast: true,
  //           });
  //           this.filterTasks(); // Re-filter to reflect changes
  //         });
  //       } else {
  //         this.filterTasks(); // Ensure the dropdown resets back to the old value
  //       }
  //     });
  //   }
  // }
